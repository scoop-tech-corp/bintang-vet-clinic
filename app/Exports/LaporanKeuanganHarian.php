...
class LaporanKeuanganHarian implements FromView, WithTitle
{
    ...
    public function view(): View
    {
        ...
        if ($this->date) {
            $expenses = $expenses->where(DB::raw('DATE(e.date_spend)'), '=', $this->date);
        }
        ...
        foreach ($payment_method as $data_pay) {
            ...
            if ($this->date) {
                $count_item = $count_item->where(DB::raw('DATE(lopm.updated_at)'), '=', $this->date);
            }
            ...
            if ($this->date) {
                $count_service = $count_service->where(DB::raw('DATE(list_of_payment_services.updated_at)'), '=', $this->date);
            }
            ...
            if ($this->date) {
                $count_pet_shop_clinic = $count_pet_shop_clinic->where(DB::raw('DATE(ppwc.created_at)'), '=', $this->date);
            }
            ...
            if ($this->date) {
                $count_pet_shop = $count_pet_shop->where(DB::raw('DATE(pp.created_at)'), '=', $this->date);
            }
